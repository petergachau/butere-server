...
import TenderForm from '../models/tenderform.js';


export const createTenderForm = async (req, res) => {
  const tour = req.body;
  const newTour = new TenderForm({
    ...tour,
    creator: req.userId,
...
export const getTenderForm= async (req,res)=>{
    
    try {
       const student=  await TenderForm.find()
    res.json(student) 
    } catch (error) {
        res.status(404).json({ message: "Something went wrong" });
